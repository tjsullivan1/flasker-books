...
def test_add_user(test_app: Flask, test_database: SQLAlchemy):
...
def test_add_user_invalid_json_keys(test_app: Flask, test_database: SQLAlchemy):
    client = test_app.test_client()
    resp = client.post(
        "/api/users",
...
def test_add_user_duplicate_email(test_app: Flask, test_database: SQLAlchemy):
    client = test_app.test_client()
    client.post(
        "/api/users",
        data=json.dumps({"username": "michael", "email": "michael@testdriven.io"}),
...
def test_remove_user(test_app, test_database, add_user):
    ...
    client = test_app.test_client()
    resp_one = client.get("/api/users")
    data = json.loads(resp_one.data.decode())
    assert resp_one.status_code == 200
    assert len(data) == 1

    resp_two = client.delete(f"/api/users/{user.id}")
    data = json.loads(resp_two.data.decode())
    assert resp_two.status_code == 200
    assert "remove-me@testdriven.io was removed!" in data["message"]

    resp_three = client.get("/api/users")
...
def test_update_user(test_app, test_database, add_user):
    ...
    resp_two = client.get(f"/api/users/{user.id}")
    data = json.loads(resp_two.data.decode())
    assert resp_two.status_code == 200
